...
class Network(object):
    def __init__(self, sizes, ceptron):
    ...
    def feedforward(self, a):
        ...
        for b, w in zip(self.biases, self.weights):
            a = self.ceptron.core_func(np.dot(w, a)+b)
        return a
    ...
    def forward(self, x):
        ...
        for b, w in zip(self.biases, self.weights):
            ...
            zs.append(z)
            outputs.append(self.ceptron.core_func(z))
        return outputs, zs
    ...
    def backprop(self, x, y):
        ...
        for i in range(1, self.num_layers):
            ...
            if i == 1: # last layer
                ...
                # print(nabla_w[-i+1].shape, delta[-i+1].shape)
                delta = self.weights[-i+1].T.dot(delta) \
                                * self.ceptron.core_func_derivative(zs[-i])

            nabla_b[-i] = delta
            pre_outputs = activations[-i-1]
            # print(delta[i].shape, pre_outputs.shape)
            nabla_w[-i] = delta.dot(pre_outputs.T)
...
net.SGD(training_data, 30, 13, 3, test_data=test_data)